...
import { User, Phone, Fingerprint, ShieldCheck, CheckCircle2, LogOut } from 'lucide-react';
import { EmptyState } from '../common/EmptyState';

export function ProfilePage({ currentUser, onOpenAuth, onLogout }) {
  if (!currentUser) {
    return (
      <EmptyState
        title="No Citizen Profile Found"
        description="Please log in with your phone number to access your verified profile."
    ...
    <div className="max-w-md mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-2 pb-6 border-b border-slate-100">
          ...
          <div>
            <h3 className="font-bold text-lg text-slate-900">{currentUser.name || 'Citizen'}</h3>
          ...
          <div>
            {currentUser.is_verified ? (
              ...
              <button
                onClick={onOpenAuth}
                className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 text-amber-800 font-bold px-3 py-1 rounded-full text-xs hover:bg-amber-100"
              >
                <span>Aadhaar Verification Pending</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 text-xs">
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
            ...
            <span className="font-bold text-slate-900">{currentUser.name || 'Not provided'}</span>
          </div>
